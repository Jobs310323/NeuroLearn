...
export async function probeModel(ref: ModelRef, timeoutMs = 90_000): Promise<ProbeResult> {
  ...
  try {
    const stream = streamObject({
      ...
      // Зонд отвечает на вопрос «отвечает ли провайдер прямо сейчас», а не
      // «удастся ли достучаться с третьей попытки». Повторы тут только тратят
      // квоту и втрое растягивают замер времени, который и есть цель зонда.
      maxRetries: 0,
      // Свой обработчик — вместо стандартного, который печатает в консоль весь
      // объект ошибки вместе с заголовками ответа и cookie Cloudflare. Причина
      // и так берётся из `fullStream` ниже и печатается одной строкой.
      onError: () => {},
    });
